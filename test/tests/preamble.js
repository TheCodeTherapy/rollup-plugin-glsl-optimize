...
export function preambleTests() {
  describe("Preamble", function () {
    describe("#insertPreamble()", function () {
      const preambleTest = (input, insert) =>
        preamble.insertPreamble(input, insert).code;
      const preambleTokenTest = (input, insert) =>
        preamble.test.insertPreambleTokens(
          ...
          (fixupLineNo) => ({
            ...
            type: 4,
            value: "",
            text: `@${fixupLineNo}:${insert}\n`,
          })
        ).code;
      it("should throw error with version directive on wrong line", function () {
        const input = "\n#version 200";
        assert.throws(
          () => preambleTest(input, "test"),
          /#version directive must be on first line/
        );
      });
    ...
    describe("#insertExtensionPreamble()", function () {
      const preambleExtTest = (input) =>
        preamble.insertExtensionPreamble(
          input,
          "test.frag",
          () => "300 es",
          "preamble"
      ...
      const extTest = (input) =>
        preamble.insertExtensionPreamble(input, "test.frag").code;
      ...
      it("should error with disabled #extension directive", function () {
        const input =
          ...
          "A\n";
        assert.throws(
          () => preambleExtTest(input),
          /extension GL_GOOGLE_include_directive cannot be disabled/
        );
      });
    ...
    describe("#fixupDirectives()", function () {
      it("should strip include and line directives", function () {
        ...
        const result = preamble.fixupDirectives(
          input,
          false,
          true,
          false,
      ...
      it("should warn about missing extension directives", function () {
        ...
        assert.include(
          unMockConsoleError(),
          `couldn't find GL_GOOGLE_cpp_style_line_directive`
        );
        assert.strictEqual(result, expected);
      });
