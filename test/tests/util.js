...
export function utilTests() {
  describe("Utility functions", function () {
    it("should correctly normalize a github release URL", function () {
      const baseURL =
        "https://github-releases.githubusercontent.com/123456/123456-123456";
      const baseParams =
        "actor_id=0&key_id=0&repo_id=123456&" +
    ...
    it("should warn about missing Khronos binaries", function () {
      process.env.GLSLANG_VALIDATOR =
        process.env.GLSLANG_OPTIMIZER =
        process.env.GLSLANG_CROSS =
          "/does/not/exist";
      assert.throws(
        () => tools.configureTools({}),
        /Khronos tool binaries could not be found/i
      );
      delete process.env.GLSLANG_VALIDATOR;
      delete process.env.GLSLANG_OPTIMIZER;
      delete process.env.GLSLANG_CROSS;
    });
    it("should output stderr from failing command", async function () {
      ...
      await assert.isRejected(
        tools.runToolBuffered("ping", ".", "ping", ["-"]),
        /failed/i
      );
