...
(async function main() {
  ...
  const downloadURL = new URL(
    `${downloadTag}/${downloadArchiveName}`,
    downloadBaseURL
  ).toString();

  const downloadArchivePath = path.join(
    binSource.folderPath,
    downloadArchiveName
  );
  ...
  try {
    await downloadFile(
      downloadURL,
      downloadArchivePath,
      downloadProgress(`binaries for ${binSource.tag}`)
    );

    const unzippedFiles = unzipAndDelete(
      downloadArchivePath,
      binSource.folderPath
    );
    ...
    console.log(err);
    fatalError(
      `Failed to download binaries for ${binSource.tag}\n${err.message}`
    );
    return;
  }
