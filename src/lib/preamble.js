 ...
 */
export function insertExtensionPreamble(
  code,
  filePath,
  versionReplacer = (v) => v,
  extraPreamble
 ...
 * @return {string}
 */
export function fixupDirectives(
  code,
  preserve = false,
  required = true,
...
function insertPreambleTokens(
  ...
  return {
    code: [
      .../** @return {Generator<import('./parse.js').ParserToken>} */ (function* () {
          ...
          switch (token.type) {
            ...
            case TOK.EOF:
              if (acceptVersion) {
                ...
                if (!didInsertion) {
                  // EOF directly after version directive
                  didInsertion = true;
                  // Needs a new line
                  yield {
                    type: TOK.EOL,
            ...
            case TOK.EOL:
              if (acceptVersion) {
                // Zero-length first line
                yield* newVersionPreambleTokens(token);
              } else {
                if (!didInsertion) {
                  // Newline after version directive, insert after this EOL
                  insertNext = true;
                  didInsertion = true;
                }
              }
            ...
            */
            ...
            default:
              if (acceptVersion) {
                // Some other token on first line
                yield* newVersionPreambleTokens(token);
              }
          }
          yield token;
