...
/** @internal */
export const allToolInfo = () =>
  Object.values(ToolConfig).map(toolInfo).join("\n");
...
export function configureTools(
  options,
  required = /** @type {GLSLToolVals[]} */ (Object.keys(ToolConfig))
) {
 ...
 * @param {boolean} [echo]
 */
export async function waitForToolBuffered(
...
export async function runToolBuffered(path, workingDir, title, args) {
  const toolResult = await waitForToolBuffered(
    launchToolPath(path, workingDir, args)
  );
  if (toolResult.error) {
